...
def compute_portvals( orders, start_val = 1000000, commission=9.95, impact=0.005, symbol = 'JPM'):
    ...
    for index, row in orders.iterrows():
        ...
        sym = 'JPM'
        shares = row['Shares']
        netholdings = Stock_Transaction[sym].sum()
        if shares == 1000 and netholdings == -1000:
            Stock_Transaction[sym][date] += 2000
            Stock_Transaction['CASH'][date] -= (prices[sym][date] * 2000 * (1 + impact)) + commission
        elif shares == 1000 and netholdings == 0:
            Stock_Transaction[sym][date] += 1000
            Stock_Transaction['CASH'][date] -= (prices[sym][date] * 1000 * (1 + impact)) + commission
        elif shares == 1000 and netholdings == 1000:
            Stock_Transaction[sym][date] += 0
            Stock_Transaction['CASH'][date] -= (prices[sym][date] * 0 * (1 + impact)) + commission
        elif shares == -1000 and netholdings == -1000:
            Stock_Transaction[sym][date] -= 0
            Stock_Transaction['CASH'][date] += (prices[sym][date] * 0 * (1 - impact)) - commission
        elif shares == -1000 and netholdings == 0:
