...
class database_activity:
    ...
    def data_etl(self,Database_Folder,good_data_folder,file_object,DatabaseName,column_names,column_datatypes,table_name,data_folder):
        try:
            ### DATA TRANSFORMATION ###
            files = glob.glob(good_data_folder+"*.csv", recursive=True)
            ...
            self.activity_log_obj.log(file_object, "%s: Quotes added successfully to Varchar columns!!")
            ...
            col_name_dtype = col_name_dtype[:-1] ## to remove the last comma
            ...
            ### DATA INSERTION ###
            table_data = pd.read_sql_query("SELECT distinct DATE from " + table_name, conn)
            insert_data = df_combined.copy()
            policy_number = list(table_data['DATE'])
...
if __name__ == "__main__":
    ...
    DatabaseName = 'WeatherVisibility'
